...
export async function createRoom(unsafeData: z.infer<typeof createRoomSchema>) {
    ...
    const {data: room, error: roomError} = await supabase.from("chat_room").insert({ name: data.name, is_public: data.isPublic }).select("id").single()

    if(roomError || room == null) {
        return { error: true, message: "Failed to create room" }
    }

    const { error: membershipError } = await supabase.from("chat_room_member").insert({ chat_room_id: room.id, member_id: user.id })
